...
function getMealType(istTime) {
     ...
     const snackEnd = 19 * hour;
     const dinnerStart = 18 * hour + 45 * minute;
     const dinnerEnd = 23 * hour + 45 * minute;
     let meal_type = "error";
...
async function getStudentDetails(qrCode) {
     // let barcode;
     const barcodeRegex = /ST(BT[2-5][0-9]{5,}|(MT|MCA)[2-5][0-9]{3,6})/;
     const match = qrCode.match(barcodeRegex);
     if (match) {
          const admission_no = match[2];
     ...
     try {
          ...
          if (!insertError) {
               await SendNotification(
                    student_id,
                    "Meal Logged",
                    `Your ${meal_type} has been logged successfully!`,
               );
          }
